# Copyright (c) 2026
# Unitree Go2 Scripts: Windows Task1 正式训练入口。
#
# 本文件用于启动 Task1 平地运动任务的 Windows 正式训练流程。
# 主要职责:
#   1. 复用 scripts/windows/_common.ps1 解析 ProjectRoot、IsaacLabRoot、PythonBat 和 LogRoot；
#   2. 调用 Task1 的 Python 训练入口 task1_train.py；
#   3. 使用适合单机 Windows GPU 的默认训练参数；
#   4. 通过 approval gate 避免误启动长时间训练。
#
# 使用方式:
#   $env:GO2_TASK1_WINDOWS_TRAIN_APPROVED = "1"
#   powershell -ExecutionPolicy Bypass -File scripts/windows/train_task1.ps1 -IsaacLabRoot <path-to-IsaacLab>
#
# Unitree Go2 Scripts: Windows Task1 formal training entry.
#
# This file launches the Windows formal training pipeline for Task1 flat locomotion.
...
#   3. Use default training parameters suitable for a single Windows GPU machine;
...
if ([string]::IsNullOrWhiteSpace($RunName)) {
    $RunName = "win_task1_train_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}

Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task1 formal training"
Write-Host "============================================================"
...
Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task1 training finished with exit code: $ExitCode"
