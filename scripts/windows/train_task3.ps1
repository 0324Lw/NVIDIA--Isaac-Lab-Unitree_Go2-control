# Copyright (c) 2026
# Unitree Go2 Scripts: Windows Task3 正式训练入口。
#
# 本文件用于启动 Task3 导航避障任务的 Windows 正式训练流程。
# 主要职责:
#   1. 复用 scripts/windows/_common.ps1 解析 ProjectRoot、IsaacLabRoot、PythonBat 和 LogRoot；
#   2. 调用 Task3 的 Python 训练入口 task3_train.py；
#   3. 保留 Task2 / Task1 checkpoint warm-start 和 resume 的可选入口；
#   4. 使用当前稳定维度: actor obs = 208，privileged obs = 276，lidar rays = 60；
#   5. 通过 approval gate 避免误启动长时间训练。
#
# 使用方式:
#   $env:GO2_TASK3_WINDOWS_TRAIN_APPROVED = "1"
#   powershell -ExecutionPolicy Bypass -File scripts/windows/train_task3.ps1 -IsaacLabRoot <path-to-IsaacLab> -PretrainedTask2 <checkpoint>
#
# Unitree Go2 Scripts: Windows Task3 formal training entry.
#
# This file launches the Windows formal training pipeline for Task3 navigation and obstacle avoidance.
...
#   3. Keep optional Task2 / Task1 checkpoint warm-start and resume entries;
...
if ([string]::IsNullOrWhiteSpace($RunName)) {
    $RunName = "win_task3_train_" + (Get-Date -Format "yyyyMMdd_HHmmss")
}

Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task3 formal training"
Write-Host "============================================================"
...
Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task3 training finished with exit code: $ExitCode"
