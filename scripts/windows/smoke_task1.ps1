# Copyright (c) 2026
# Unitree Go2 Scripts: Windows Task1 快速训练检查入口。
#
# 本文件用于启动 Task1 平地运动任务的 Windows 最小规模 smoke 训练。
# 主要职责:
#   1. 复用 scripts/windows/_common.ps1 解析 ProjectRoot、IsaacLabRoot、PythonBat 和 LogRoot；
#   2. 调用 Task1 的 Python 训练入口 task1_train.py；
#   3. 使用较小 num-envs 和 total-env-steps，验证 IsaacLab、skrl、Go2 环境和 PPO 训练链路；
#   4. 通过 approval gate 避免误启动训练。
#
# 使用方式:
#   $env:GO2_TASK1_WINDOWS_SMOKE_APPROVED = "1"
#   powershell -ExecutionPolicy Bypass -File scripts/windows/smoke_task1.ps1 -IsaacLabRoot <path-to-IsaacLab>
#
# Unitree Go2 Scripts: Windows Task1 smoke training entry.
#
# This file launches a minimal Windows smoke training run for Task1 flat locomotion.
...
#   3. Use small num-envs and total-env-steps to validate the IsaacLab, skrl, Go2 environment, and PPO training pipeline;
...
Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task1 smoke training"
Write-Host "============================================================"
...
Write-Host "============================================================"
Write-Host "Unitree Go2 Windows Task1 smoke finished with exit code: $ExitCode"
