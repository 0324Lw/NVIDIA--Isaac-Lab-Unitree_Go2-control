# Copyright (c) 2026
# Unitree Go2 Scripts: Windows Task3 GUI 可视化入口。
#
# 本文件用于在 Windows 下以 GUI 方式可视化 Task3 导航避障模型。
# 主要职责:
#   1. 接收用户传入的 checkpoint 或使用 eval_task3.ps1 的自动 checkpoint 查找逻辑；
#   2. 调用 scripts/windows/eval_task3.ps1；
...
#
# Unitree Go2 Scripts: Windows Task3 GUI visualization entry.
#
# This file visualizes a Task3 navigation and obstacle avoidance model with the Windows GUI.
# Main responsibilities:
#   1. Accept a user-provided checkpoint or reuse the automatic checkpoint discovery in eval_task3.ps1;
