# Copyright (c) 2026
# Unitree Go2 Scripts: Windows Task2 GUI 可视化入口。
#
# 本文件用于在 Windows 下以 GUI 方式可视化 Task2 多地形运动模型。
# 主要职责:
#   1. 接收用户传入的 checkpoint 或使用 eval_task2.ps1 的自动 checkpoint 查找逻辑；
#   2. 调用 scripts/windows/eval_task2.ps1；
#   3. 支持通过 StartK 指定课程进度评估点；
#   4. 默认不传入 -HeadlessEval，使 Isaac Sim 以 GUI 模式运行。
#
# 使用方式:
...
#
# Unitree Go2 Scripts: Windows Task2 GUI visualization entry.
#
# This file visualizes a Task2 multi-terrain locomotion model with the Windows GUI.
